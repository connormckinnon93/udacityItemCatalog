	...
	if 'username' not in login_session:
		...
		if request.method == 'POST':
			try:
				...
				session.add(newItem)
				session.commit()
				# Let the user now it was successfully added and then redirect to frontpage
...
def updateItem(category_id, item_id):
	...
	if login_session['user_id'] != itemToEdit.user_id:
		# Let them know they don't have permission and give them a chance to change accounts
		...
		if request.method == 'POST':
			...
			# If there is an error let them know and then retry
				...
				return redirect(url_for('updateItem', item_id = itemToEdit.id), category_id = category_id)
...
def deleteItem(category_id, item_id):
	# Check for user
	if 'username' not in login_session:
		# If no user present tell them they need to login first and then let them try
		flash('You need to login first!')
		return redirect(url_for('showLogin'))
	# Load the item to be deleted
	itemToDelete = session.query(Item).filter_by(id=item_id).one()
	# Check if user is owner of item and if not notify them and let them change account
	...
	# If user is owner
		...
		if request.method == 'POST':
			...
			# if error occurs let them know and then let them try again
				...
				return redirect(url_for('deleteItem', item_id = itemToDelete.id, category_id = category_id))
...
def gconnect():
    ...
    # ADD PROVIDER TO LOGIN SESSION (this can be expanded to include other providers later)
    ...
    # Create an HTML response (this seems like a poo method of doing so... is it possible to use JINJA2)
    output = ''
    output += '<h1>Welcome, '
